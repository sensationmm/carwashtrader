...
if(substr($pageReferrer,0,4) != 'supp')
	unset($_SESSION);
	...
	<!-- Home -->
        ...
        <article>
        	...
        	<div class="filterBox">
                ...
                <div class="viewToggle"><a href="/suppliers/map-view/#listings" title="Go to list view">Map view</a></div>
            </div>
            ...
            <?php
				if(isset($_GET["showID"])) {
					...
					$geocode = get_field('supplier_geocode', $listingID);
					...
					if($searchPostcode != '' && $searchPostcode != 'Required for proximity search, eg. SW1, GU6, CO1') {
						...
						for($r=0; $r<sizeof($results); $r++) {
							...
							$listingLoc = get_field('supplier_postcode', $listingID);
					...
					if(sizeof($results) > 0) {
						...
						for($r=$start; $r<$end; $r++) {
							...
							if($listingID != '') {
								...
								echo outputSupplierListing($listingID,$title,$description,$address,$postcode,$geocode,$contact,$website,$openinghours,$prices,$distance,$doDistance);
							}
							...
							if($a > 0 && ($a+1)<15 && (($a+1)%5) == 0 && sizeof($results) > $advertCount*5) {
								$advertCount++;
								echo outputAdverts('supplier-listings', 'in-listing', 3, $advertCount); 
							}
							//End adverts
						}

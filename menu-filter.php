                ...
                <?php
					...
					for($t=0; $t<sizeof($types); $t++)
					{
						...
						*/
						...
						$ancestors = get_ancestors( $types[$t]->term_id, 'cwt_prodsupp_cats' );

						if((sizeof($ancestors) == 0 || $types[$t]->parent == $filterSet) && $checkRightPosts > 0) {

							if($types[$t]->term_id != '') {
								echo '<li';
								if($types[$t]->term_id == $setFilter)
									echo ' class="active"';
								echo '><a  href="" onclick="setFilter('.$types[$t]->term_id.');return false;">';
								echo $types[$t]->name.'</a></li>';

								$typesSub = get_terms($filterListRequired, array('hide_empty'=>true,'parent'=>$types[$t]->term_id,'orderby'=>'term_order')); 
